...
class Transformers:
    def __init__(
        ...
        flag_columns: list,
        unrelated_items: list,
        correlation_threshold: float,
        outliers_removal_lower_bound: float,
        outliers_removal_upper_bound: float,
    ):
        self.data = None
    ...
    def missing_values_removal(self, X):
        ...
        high_missing_cols = missing_percentages[
            missing_percentages > self.threshold
        ].index.tolist()
        high_missing_cols = [
            col for col in high_missing_cols if col not in self.excluded_attributes
        ]
        X = X.drop(columns=high_missing_cols)
        logger.info(
            f"Removed {len(high_missing_cols)} columns with missing values greater than {self.threshold}%"
        )
        return X
        ...
        for flag in flag_cols:
            ...
            if flag in X.columns and "TARGET" in X.columns:
                try:
                    ...
                    if (
                        pd.isna(correlation)
                        or abs(correlation) < self.correlation_threshold
                    ):
                        X = X.drop(columns=flag)
                        logger.info(
                            f"Removed {flag} column with correlation less than {self.correlation_threshold}"
                        )
    ...
    def outliers_removal(self, X):
        ...
        for col in num_cols:
            ...
            logger.info(
                f"Column '{col}': {num_outliers} outliers capped at [{lower_bound:.2f}, {upper_bound:.2f}]"
            )
        return X

    def fit_imputer(self, X_train, numeric_cols):
        X_train = X_train.copy()
        self.car_owners_median = X_train.loc[
            X_train["FLAG_OWN_CAR"] == "Y", "OWN_CAR_AGE"
        ].median()
        self.impute_cols = [
            col for col in numeric_cols if col not in ["OWN_CAR_AGE", "TARGET"]
        ]
        self.imputer = SimpleImputer(strategy="median")
        self.imputer.fit(X_train[self.impute_cols])
        logger.info(
            f"Fitted imputer on training data. Car owners median age: {self.car_owners_median}"
        )
        return self
    ...
    def transform_imputer(self, X):
        ...
        X.loc[(X["FLAG_OWN_CAR"] == "Y") & (X["OWN_CAR_AGE"].isna()), "OWN_CAR_AGE"] = (
            self.car_owners_median
        )
    ...
    def fit_encoder(self, X_train, y_train, categorical_cols):
        ...
        if "NAME_EDUCATION_TYPE" in categorical_cols:
            ...
            logger.info(
                "Fitted ordinal encoder for 'NAME_EDUCATION_TYPE' on training data"
            )

        # One-hot encoding - only learn column struct only
        self.one_hot_cols = [
            col
            for col in ["NAME_FAMILY_STATUS", "NAME_HOUSING_TYPE", "NAME_INCOME_TYPE"]
        ...
        for col in self.one_hot_cols:
            ...
            X_temp = pd.get_dummies(
                X_temp, columns=[col], prefix=col, drop_first=True, dtype=int
            )
        ...
        for col in self.target_encode_cols:
            ...
            logger.info(
                f"Target encoded '{col}' with mean target values for training data"
            )
    ...
    def transform_encoder(self, X):
        ...
        if "NAME_EDUCATION_TYPE" in self.ordinal_encoders:
            X["NAME_EDUCATION_TYPE"] = self.ordinal_encoders[
                "NAME_EDUCATION_TYPE"
            ].transform(X[["NAME_EDUCATION_TYPE"]])
        ...
        for col in self.one_hot_cols:
            ...
            logger.info(
                f"One-hot encoded '{col}' into {len(X.columns) - before_cols} new columns"
            )
        ...
        if "NAME_CONTRACT_TYPE" in X.columns:
            ...
            logger.info(
                f"Mapped 'NAME_CONTRACT_TYPE' with values {self.contract_mapping}"
            )
        ...
        for col in binary_cols:
            if col in X.columns:
                ...
                logger.info(
                    f"Mapped binary column '{col}' with values {self.binary_mapping}"
                )
...
#     application_data.load_data(data_path)
