...
class DataCleaner:
    ...
    def fix_family_size(self):
        ...
        before = len(self.data)
        self.data = self.data[
            self.data["CNT_FAM_MEMBERS"] >= self.data["CNT_CHILDREN"] + 1
        ]
        removed = before - len(self.data)
        ...
        logger.info(
            f"Capped {capped_count} rows with family members at {fam_cap} (95th percentile), max before cap was {before_cap}"
        )
        return self
    ...
    def replace_org_name(self):
        ...
        logger.info(
            f"Replacing {xna_cols} 'XNA' values in ORGANIZATION_TYPE with 'Undefined org. type'"
        )
        self.data["ORGANIZATION_TYPE"] = self.data["ORGANIZATION_TYPE"].replace(
            "XNA", "Undefined org. type"
        )
        return self
    ...
    def transform_days(self):
        ...
        self.data["YEARS_ID_PUBLISH"] = (-self.data["DAYS_ID_PUBLISH"] / 365).astype(
            int
        )
        logger.info(
            "Transformed DAYS_BIRTH to AGE, DAYS_EMPLOYED to YEARS_EMPLOYED, and DAYS_ID_PUBLISH to YEARS_ID_PUBLISH"
        )
    ...
    def cap_income(self):
        ...
        self.data["AMT_INCOME_TOTAL"] = np.clip(
            self.data["AMT_INCOME_TOTAL"], None, income_cap
        )
        logger.info(
            f"Removed {before - len(self.data)} rows with impossible income values"
        )
    ...
    def impute_years_employed(self):
        ...
        logger.info(
            f"Identified {missing_employment} rows with -1000 in YEARS_EMPLOYED"
        )
        self.data["YEARS_EMPLOYED"] = self.data["YEARS_EMPLOYED"].replace(-1000, np.nan)
        median_years = self.data["YEARS_EMPLOYED"].median()
        self.data["YEARS_EMPLOYED"] = (
            self.data["YEARS_EMPLOYED"].fillna(median_years).astype(int)
        )
        logger.info(
            f"Imputed missing values in YEARS_EMPLOYED with median value: {median_years}"
        )
        logger.info(
            f"Unique values in YEARS_EMPLOYED after imputation: {self.data['YEARS_EMPLOYED'].unique()}"
        )
        return self

    def clean_occupation_type(self):
        self.data["OCCUPATION_TYPE"] = self.data["OCCUPATION_TYPE"].fillna("Unknown")
        self.data["OCCUPATION_TYPE"] = self.data["OCCUPATION_TYPE"].replace(
            "Unknown", "Undefined occupation"
        )
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Cleaned data has {len(cleaned_data)} rows and {len(cleaned_data.columns)} columns"
    )
